...
gcloud run deploy greenchain-agent \
  ...
  --memory 1Gi \
  --set-env-vars "GOOGLE_GENAI_USE_VERTEXAI=TRUE,GOOGLE_CLOUD_LOCATION=$REGION,GOOGLE_CLOUD_PROJECT=$PROJECT,GEMINI_MODEL=gemini-3.5-flash,GREENCHAIN_STORE=file,GREENCHAIN_FIXTURES=/app/bundled_fixtures,GREENCHAIN_COMPANY_ID=northwind-energy"
...
gcloud run deploy greenchain-web \
  ...
  --project "$PROJECT" \
  --allow-unauthenticated \
  --set-env-vars "AGENT_URL=$AGENT_URL,NEXT_PUBLIC_COMPANY_ID=northwind-energy"
